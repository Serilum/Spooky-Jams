...
public class SlimesSquishJamItem extends JamItem {
	...
	protected List<Pair<MobEffect, Integer>> getConsumeEffects() {
		return List.of(
			Pair.of(MobEffects.JUMP, 6000),
			Pair.of(MobEffects.ABSORPTION, 1200),
			Pair.of(MobEffects.SLOW_FALLING, 300),
			Pair.of(MobEffects.MOVEMENT_SLOWDOWN, 160)
		);
	}

	public @NotNull Component getName(@NotNull ItemStack stack) {
		return Component.translatable(this.getDescriptionId(stack))
			.setStyle(Style.EMPTY.withColor(TextColor.fromRgb(0xc9f573)));
	}
	...
	public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
		...
		Style style = Style.EMPTY.withColor(TextColor.fromRgb(0x8ec43a)).withItalic(true);
