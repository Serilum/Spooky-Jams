...
public class EndermansEssenceJamItem extends JamItem {
	...
	protected List<Pair<MobEffect, Integer>> getConsumeEffects() {
		return List.of(
			...
			Pair.of(MobEffects.MOVEMENT_SPEED, 1200)
		);
	}

	public @NotNull Component getName(@NotNull ItemStack stack) {
		return Component.translatable(this.getDescriptionId(stack))
			.setStyle(Style.EMPTY.withColor(TextColor.fromRgb(0x7f5ed1)));
	}
	...
	public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
		...
		Style style = Style.EMPTY.withColor(TextColor.fromRgb(0x5a3e9e)).withItalic(true);
